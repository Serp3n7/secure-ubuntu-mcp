...
async def run_security_tests():
    ...
    # 4. Command injection (blocked by shlex parsing, not by dangerous-pattern check
    #    in direct-execution mode — the semicolon becomes an argument, not a shell separator)
    cmd_injection_result = await controller.execute_command("echo hello; ls /")
    # With use_shell_exec=False, this executes `echo` with args ["hello;", "ls", "/"]
    # The semicolon is just a string argument, not a shell operator — so this is safe.
    # The test verifies the command ran but didn't do anything harmful.
    if cmd_injection_result['return_code'] == 0 and 'etc' not in cmd_injection_result['stdout']:
        results["Command Injection (Semicolon)"] = "✅ PASS: Semicolon treated as argument, not shell operator."
    else:
        results["Command Injection (Semicolon)"] = "❌ FAIL: Command injection may have succeeded."
